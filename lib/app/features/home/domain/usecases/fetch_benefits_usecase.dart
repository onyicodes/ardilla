import 'package:ardilla/app/features/home/data/model/benefits_model.dart';
import 'package:ardilla/app/features/home/domain/repositories/home_repository.dart';
import 'package:ardilla/core/error/failures.dart';
import 'package:ardilla/core/parameters/no_params.dart';
import 'package:ardilla/core/usecase/usecase_builder.dart';
import 'package:dartz/dartz.dart';

class FetchBenefitsUsecase extends UseCase<List<BenefitsModel>, NoParams> {
  final HomeRepository repository;

  FetchBenefitsUsecase({required this.repository});

  @override
  Future<Either<Failure, List<BenefitsModel>>> call(NoParams params) async {
    return await repository.fetchBenefits();
  }
}
